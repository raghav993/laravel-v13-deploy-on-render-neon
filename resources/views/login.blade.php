...
<body>
    ...
    <div class="container py-4">
      <div class="row g-0 justify-content-center shadow-lg rounded-4 overflow-hidden">
        <div class="col-lg-5 d-none d-lg-flex text-white p-5 flex-column justify-content-between" style="background:linear-gradient(150deg,#16302e,#2f6e68)">
          <div><a href="{{ route('home') }}"><img src="{{ asset('assets/img/light-logo.png') }}" alt="Sahayika" width="160"></a><br>
            <span class="mt-2 badge rounded-pill bg-warning text-dark mt-5 mb-3">घर की मदद • INDORE</span>
          ...
          <div class="rounded-4 overflow-hidden mt-4">
            <img src="{{ asset('assets/img/login-bg1.jpeg') }}" class="rounded-4 w-100" style="object-fit:cover" alt="Sahayika">
          </div>
        </div>
        <div class="col-lg-7 bg-white p-4 p-md-5">
          ...
          <div class="mb-4"><span class="badge bg-success-subtle text-success-emphasis rounded-pill mb-2"><i class="bi bi-person-check me-1"></i> Welcome back</span>
            <h2 class="serif fw-bold mb-1">अपने account में login करें</h2>
            <p class="text-secondary mb-0">Account type चुनें और आगे बढ़ें।</p>
          </div>
          @if($errors->any())
          <div class="alert alert-danger border-0"><i class="bi bi-exclamation-circle me-2"></i>
          ...
          <form method="POST" action="{{ route('login.store') }}">
            ...
            <div class="mb-3"><label for="identifier" class="form-label fw-semibold">Email या mobile number</label>
              <div class="input-group"><span class="input-group-text"><i class="bi bi-person"></i></span><input class="form-control" id="identifier" name="identifier" value="{{ old('identifier') }}" placeholder="demo.customer01@sahayika.test" required></div>
            </div>
            <div class="mb-3"><label for="password" class="form-label fw-semibold">Password</label>
              <div class="input-group"><span class="input-group-text"><i class="bi bi-lock"></i></span><input class="form-control" id="password" type="password" name="password" placeholder="Your password" required></div>
            </div>
  ...
  <script>
    function setRole(role, button) {
      ...
      document.querySelectorAll('.role').forEach(x => x.classList.remove('active', 'border-success', 'bg-success-subtle'));
      button.classList.add('active', 'border-success', 'bg-success-subtle')
    }
  </script>
</body>
